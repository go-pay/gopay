...
func (w *Client) UnifiedOrder(ctx context.Context, bm gopay.BodyMap) (wxRsp *UnifiedOrderResponse, resBm gopay.BodyMap, header http.Header, err error) {
	...
	var bs []byte
	if w.IsProd {
		bs, header, err = w.doProdPost(ctx, bm, unifiedOrder, nil)
	} else {
...
func (w *Client) Micropay(ctx context.Context, bm gopay.BodyMap) (wxRsp *MicropayResponse, resBm gopay.BodyMap, header http.Header, err error) {
	...
	var bs []byte
	if w.IsProd {
		bs, header, err = w.doProdPost(ctx, bm, microPay, nil)
	} else {
		bm.Set("total_fee", 1)
...
func (w *Client) QueryOrder(ctx context.Context, bm gopay.BodyMap) (wxRsp *QueryOrderResponse, resBm gopay.BodyMap, header http.Header, err error) {
	err = bm.CheckEmptyError("nonce_str")
	if err != nil {
		return nil, nil, nil, err
	}
	if bm.GetString("out_trade_no") == util.NULL && bm.GetString("transaction_id") == util.NULL {
		return nil, nil, nil, errors.New("out_trade_no and transaction_id are not allowed to be null at the same time")
	}
	var bs []byte
	if w.IsProd {
		bs, header, err = w.doProdPost(ctx, bm, orderQuery, nil)
	} else {
		bs, header, err = w.doSanBoxPost(ctx, bm, sandboxOrderQuery)
	}
	if err != nil {
		return nil, nil, header, err
	}
	...
	if err = xml.Unmarshal(bs, wxRsp); err != nil {
		return nil, nil, header, fmt.Errorf("xml.UnmarshalStruct(%s)：%w", string(bs), err)
	}
	resBm = make(gopay.BodyMap)
	if err = xml.Unmarshal(bs, &resBm); err != nil {
		return nil, nil, header, fmt.Errorf("xml.UnmarshalBodyMap(%s)：%w", string(bs), err)
...
func (w *Client) CloseOrder(ctx context.Context, bm gopay.BodyMap) (wxRsp *CloseOrderResponse, resBm gopay.BodyMap, header http.Header, err error) {
	...
	var bs []byte
	if w.IsProd {
		bs, header, err = w.doProdPost(ctx, bm, closeOrder, nil)
	} else {
		bs, header, err = w.doSanBoxPost(ctx, bm, sandboxCloseOrder)
	}
	if err != nil {
		return nil, nil, header, err
	}
...
func (w *Client) Refund(ctx context.Context, bm gopay.BodyMap) (wxRsp *RefundResponse, resBm gopay.BodyMap, header http.Header, err error) {
	...
	if w.IsProd {
		...
		bs, header, err = w.doProdPost(ctx, bm, refund, tlsConfig)
	} else {
		bs, header, err = w.doSanBoxPost(ctx, bm, sandboxRefund)
	}
	if err != nil {
		return nil, nil, header, err
	}
...
func (w *Client) QueryRefund(ctx context.Context, bm gopay.BodyMap) (wxRsp *QueryRefundResponse, resBm gopay.BodyMap, header http.Header, err error) {
	err = bm.CheckEmptyError("nonce_str")
	if err != nil {
		return nil, nil, nil, err
	}
	if bm.GetString("refund_id") == util.NULL && bm.GetString("out_refund_no") == util.NULL && bm.GetString("transaction_id") == util.NULL && bm.GetString("out_trade_no") == util.NULL {
		return nil, nil, nil, errors.New("refund_id, out_refund_no, out_trade_no, transaction_id are not allowed to be null at the same time")
	}
	var bs []byte
	if w.IsProd {
		bs, header, err = w.doProdPost(ctx, bm, refundQuery, nil)
	} else {
		bs, header, err = w.doSanBoxPost(ctx, bm, sandboxRefundQuery)
	}
	if err != nil {
		return nil, nil, header, err
	}
...
func (w *Client) Reverse(ctx context.Context, bm gopay.BodyMap) (wxRsp *ReverseResponse, header http.Header, err error) {
	...
	if w.IsProd {
		...
		bs, header, err = w.doProdPost(ctx, bm, reverse, tlsConfig)
	} else {
		bs, header, err = w.doSanBoxPost(ctx, bm, sandboxReverse)
	}
	if err != nil {
